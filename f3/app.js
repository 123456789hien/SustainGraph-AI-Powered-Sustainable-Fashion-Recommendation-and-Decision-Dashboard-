...
// Map rating letters to numeric scores (0–1) với spread rõ ràng hơn
...
// Yes=1, No=0, unknown=0.5 để policy score không bị đều đều
...
function normalizeAndComputeSIS(rawRows) {
  ...
  const rowsWithSIS = rows.map((r, i) => {
    ...
    // Environmental impact: Carbon + Water + Waste (normalised, inverted)
    const envScore = (1 - co2n + 1 - watern + 1 - wasten) / 3;
    // Policy / governance: rating + eco manufacturing + recycling
    const policyScore = (ratingScore + ecoScore + recScore) / 3;

    // Ưu tiên environmental hơn một chút
...
/* ========== "PCA" PROJECTION (USE Carbon + Water + Waste) ========== */
...
function computePcaLikeCoords(materialAgg) {
  ...
  // x: normalized Carbon
  // y: trung bình của Water và Waste
