...
/* ========== PREPROCESS + SIS (Carbon + Water + Waste) ========== */
...
/* ========== SIS REGRESSION DATA (for ML model) ========== */

function buildSISRegressionData(rowsWithSIS) {
  const X = [];
  const y = [];
  ...
  rowsWithSIS.forEach((r) => {
    ...
    const features = [
      isFinite(r.Carbon_Footprint_norm) ? r.Carbon_Footprint_norm : 0.5,
      isFinite(r.Water_Usage_norm) ? r.Water_Usage_norm : 0.5,
      isFinite(r.Waste_Production_norm) ? r.Waste_Production_norm : 0.5,
      ratingLetterToScore(r.Sustainability_Rating),
      yesNoScore(r.Eco_Friendly_Manufacturing),
      yesNoScore(r.Recycling_Programs),
      isFinite(r.Price_norm) ? r.Price_norm : 0.5,
    ];
    X.push(features);
    y.push(r.SIS);
  });
