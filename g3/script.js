...
// IMPORTANT: Auto-load URL — giữ y như bạn đang dùng
const AUTOLOAD_URL =
  "https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/g3/Kaggle_sust_dataset.csv";
...
function renderAllEDAAndForecast() {
  renderMaterialEda();
  const yearAgg = renderCountryEda();
  const yearAggForYearChart = renderYearEda();
  const yearAggBase = yearAggForYearChart && yearAggForYearChart.length
    ? yearAggForYearChart
    : yearAgg;
...
function renderMaterialEda() {
  ...
  edaMaterialNote.innerText =
    ...
    `For a sustainability-first strategy you can prioritise high-SIS materials while checking if ` +
    `their price premium fits the brand positioning.`;
}
...
function renderCountryEda() {
  ...
  edaCountryNote.innerText =
    `Country EDA insight: among the top price markets, “${bestPrice.country}” shows ` +
    `the highest average price ≈ $${bestPrice.meanPrice.toFixed(
      2
    )}, while “${bestSIS.country}” leads in SIS ≈ ${bestSIS.meanSIS.toFixed(
      2
    )}. ` +
    `This helps decide where premium sustainable collections should be positioned ` +
    `and which markets still need SIS improvement.`;

  // return for year aggregation reuse if needed
  return data;
}
...
function renderYearEda() {
  ...
  if (trend === "upward") {
    ...
    trendText =
      "SIS is relatively stable across years, so pricing or volume changes may drive most of the variation.";
  }
...
async function renderLstmForecast(yearAgg) {
  ...
  if (diff > 0.02) {
    comment =
      "The LSTM forecast suggests SIS will continue to improve over the next 3 years, " +
      "assuming past patterns hold. Brands can lock in this positive trajectory by " +
      "doubling down on high-performing materials and suppliers.";
  } else if (diff < -0.02) {
    comment =
      "The LSTM forecast indicates a soft decline in SIS in the next 3 years, which might " +
      "reflect rising costs or a shift to less sustainable inputs. It is a signal to review " +
      "suppliers and redesign product mixes.";
  } else {
    comment =
      "The LSTM forecast is roughly flat, suggesting SIS will stay close to current levels. " +
      "Any strong improvement will probably require explicit strategic interventions, not just " +
      "natural continuation of current trends.";
  }
