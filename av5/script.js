...
const elbowInsightDiv = document.getElementById("elbow-insight");
...
let MATERIAL_AGG = [];
let PCA_POINTS = [];
let ELBOW_INFO = null; // lưu kết quả Elbow (k tối ưu + inertia) cho material clustering

// IMPORTANT: Auto-load URL — dạng raw GitHub chuẩn
const AUTOLOAD_URL =
  "https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/av5/Kaggle_sust_dataset.csv";
...
function renderPcaInsight(kUsed) {
  ...
  const largestCluster = Object.entries(clusterCounts).sort(([, a], [, b]) => b - a)[0];
  
  pcaInsightDiv.innerHTML = `
    <p><strong>K-Means Clustering:</strong> Materials were grouped into <strong>${kUsed} clusters</strong> based on their environmental impact (CO₂, Water, Waste).</p>
    <p><strong>Insight:</strong> Cluster ${largestCluster[0]} is the largest with ${largestCluster[1]} materials. The plot visualizes the trade-off between CO₂ (x-axis) and combined Water/Waste (y-axis).</p>
    <p><strong>Recommendation:</strong> Focus on materials in the <strong>Low-impact cluster</strong> (typically cluster 0 or 1) for the most sustainable choices.</p>
  `;
}
...
function renderElbowCharts() {
  ...
  if (ELBOW_INFO && ELBOW_INFO.inertias && ELBOW_INFO.inertias.length > 0) {
    ...
    renderElbowInsight(ELBOW_INFO.bestK);
  }
...
function renderElbowInsight(materialK) {
  if (!elbowInsightDiv) return;
  
  elbowInsightDiv.innerHTML = `
    <p><strong>Elbow Method Result:</strong> The optimal number of clusters (k) for Materials is <strong>${materialK}</strong>. This k-value minimizes the within-cluster sum of squares (Inertia) while keeping the model simple.</p>
    <p><strong>Interpretation:</strong> Using ${materialK} clusters provides the best balance between model fit and complexity for grouping materials by environmental impact.</p>
  `;
}
...
function renderParetoInsight(materialPareto, brandPareto) {
  ...
  const bestMaterial = materialPareto.length > 0 ? materialPareto[materialPareto.length - 1] : null;
  const bestBrand = brandPareto.length > 0 ? brandPareto[brandPareto.length - 1] : null;
  
  let insight = "<p><strong>Pareto Frontier Analysis:</strong> Identifies the set of choices where no single metric (SIS or Price) can be improved without sacrificing the other.</p>";
  
  if (bestMaterial) {
    insight += `<p><strong>Top Material:</strong> <strong>${bestMaterial.name}</strong> (SIS: ${bestMaterial.sis.toFixed(2)}, Price: $${bestMaterial.price.toFixed(2)}) is on the frontier, offering a great balance.</p>`;
  }
  
  if (bestBrand) {
    insight += `<p><strong>Top Brand:</strong> A brand with SIS of ${bestBrand.sis.toFixed(2)} and Price of $${bestBrand.price.toFixed(2)} is a Pareto-optimal choice.</p>`;
  }
  
  insight += "<p><strong>Recommendation:</strong> Brands/Materials on the frontier are the most **efficient** choices for your current SIS priority.</p>";
