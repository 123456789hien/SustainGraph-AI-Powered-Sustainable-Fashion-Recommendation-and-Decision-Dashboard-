...
loadSampleBtn.addEventListener("click", () => {
  ...
  fetch("https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/ok4/Kaggle_sust_dataset.csv")
    .then((res) => res.text())
    .then((csv) => {
...
function renderParetoBrand() {
  ...
  // chỉ dùng computeParetoFlags để tìm frontier, KHÔNG tính thêm điểm ranking
  const paretoFlags = window.computeParetoFlags
    ? window.computeParetoFlags(data.map(d => ({ SIS: d.sis, Average_Price_USD: d.price })))
    : [];
  ...
  // Frontier line chỉ để visualization, không dùng để ranking
  if (paretoPoints.length > 1) {
    const sortedPareto = [...paretoPoints].sort((a, b) => a.price - b.price);
    const line = d3.line()
      .x(d => xScale(d.price))
  ...
  svg.selectAll("circle.all-point")
    ...
    .on("mouseover", function (event, d) {
      ...
      tooltip.html(`
        ...
        <div class="tooltip-footer" style="color:var(--text-muted);">
          ${isPareto ? '✓ Pareto-optimal' : 'Not on frontier'}
        </div>
      `).style("visibility", "visible");
    })
    .on("mousemove", (event) =>
...
function renderRecommendationTab(tabId, items) {
  ...
  items.forEach((item, idx) => {
    ...
    const balanceScore = item.balanceScore || 0;
    ...
    div.innerHTML = `
      <div class="reco-header">
        ...
        <div class="reco-score">Balance: ${balanceScore.toFixed(3)}</div>
      </div>
