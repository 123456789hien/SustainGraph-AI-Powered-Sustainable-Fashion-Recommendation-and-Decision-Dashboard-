...
/* ========== RECOMMENDATION ENGINE (PARETO + 3 CATEGORIES) ========== */

/**
 * Build recommendations using Multi-Objective Optimization.
 *
 * Ý tưởng:
 * - Tìm Pareto set dựa trên (Price, SIS) → computeParetoFlags(...)
 * - Trong Pareto set chia 3 góc nhìn:
 *   1) Max Sustainability  : SIS cao nhất (top theo SIS)
 *   2) Best Value          : Giá thấp nhất (top theo Price)
 *   3) Balanced Trade-off  : Gần điểm lý tưởng (SIS max, Price min) nhất
 *
 * Không xếp hạng toàn bộ Pareto theo một scalar duy nhất
 * mà trả về ba danh sách song song cho business decision.
 */
function buildRecommendations(rowsWithSIS, topNPerCategory = 5) {
  if (!rowsWithSIS || !rowsWithSIS.length) {
    return {
      maxSustainability: [],
      bestValue: [],
      balanced: [],
  ...
  // 1) Chuẩn hóa giá để dùng sau (không ảnh hưởng đến SIS gốc)
  const prices = rowsWithSIS.map(r => r.Average_Price_USD);
  const priceNormGlobal = normalizeMinMax(prices);

  const baseItems = rowsWithSIS.map((r, i) => ({
    ...r,
    priceNorm: priceNormGlobal.norm[i],
  }));

  // 2) Đánh dấu Pareto-optimal
  const paretoFlags = computeParetoFlags(baseItems); // hàm này đã có phía trên app.js
  const paretoItems = baseItems.filter((item, idx) => paretoFlags[idx]);
  ...
  // 3) Chuẩn hóa SIS và Price chỉ trong Pareto set để tính "distance to ideal"
  const sisNormLocal = normalizeMinMax(paretoItems.map(item => item.SIS));
  const priceNormLocal = normalizeMinMax(
    paretoItems.map(item => item.Average_Price_USD)
  );

  paretoItems.forEach((item, i) => {
    const sisN = sisNormLocal.norm[i];        // 0 → 1 (thấp → cao)
    const priceN = priceNormLocal.norm[i];    // 0 → 1 (rẻ → đắt)

    // Điểm lý tưởng trong không gian chuẩn hóa: SIS = 1, Price = 0
    const dx = 1 - sisN;
    const dy = priceN;

    // Khoảng cách tới điểm lý tưởng
    item.distanceToIdeal = Math.sqrt(dx * dx + dy * dy);

    // Balance score = 1 - distance (càng gần ideal càng cao)
    item.balanceScore = 1 - item.distanceToIdeal;
  });

  // 4) Category 1 — Max Sustainability (SIS cao nhất trong Pareto set)
  const maxSustainability = [...paretoItems]
    .sort((a, b) => b.SIS - a.SIS)
    .slice(0, topNPerCategory);

  // 5) Category 2 — Best Value (giá thấp nhất trong Pareto set)
  const bestValue = [...paretoItems]
    .sort((a, b) => a.Average_Price_USD - b.Average_Price_USD)
    .slice(0, topNPerCategory);

  // 6) Category 3 — Balanced Trade-off
  //    Chọn những điểm có distanceToIdeal nhỏ nhất (gần ideal nhất)
  const balanced = [...paretoItems]
    .sort((a, b) => a.distanceToIdeal - b.distanceToIdeal)
    .slice(0, topNPerCategory);

  // Không re-rank lại theo một tiêu chí khác
  // vì mỗi nhóm đã được sort đúng theo định nghĩa của nó
