...
function buildRecommendations(rowsWithSIS, topN = 5) {
  ...
  // --- FINAL ACADEMICALLY CORRECT PARETO LOGIC (Using Absolute Thresholds) ---
  
  const MAX_DISPLAY = 10;
  const BALANCED_DISPLAY = 5;
  ...
  paretoItems.forEach((item, i) => {
    ...
    // Balance Score is the inverse of distanceToIdeal (higher is better)
    item.balanceScore = 1 - item.distanceToIdeal;
  });

  // 1. Max Sustainability: SIS >= 0.9 (Absolute Threshold)
  const maxSustainability = paretoItems
    .filter(item => item.SIS >= 0.9)
    .sort((a, b) => b.SIS - a.SIS)
    .slice(0, MAX_DISPLAY);

  // 2. Best Value: Price <= 25 (Absolute Threshold)
  const bestValue = paretoItems
    .filter(item => item.Average_Price_USD <= 25)
    .sort((a, b) => a.Average_Price_USD - b.Average_Price_USD)
    .slice(0, MAX_DISPLAY);

  // 3. Balanced Trade-off: Smallest Distance-to-Ideal Metric (d)
  ...
  const balanced = [...paretoItems]
    ...
    .slice(0, BALANCED_DISPLAY);
    
  // Final sort for display within each category (optional, but good practice)
  // Note: The lists are already sorted by the filtering logic, but this ensures consistency
  maxSustainability.sort((a, b) => b.SIS - a.SIS);
  bestValue.sort((a, b) => a.Average_Price_USD - b.Average_Price_USD);
  balanced.sort((a, b) => b.balanceScore - a.balanceScore); // Sort by new balanceScore (descending)
