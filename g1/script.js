...
loadSampleBtn.addEventListener("click", () => {
  ...
  fetch("https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/g1/Kaggle_sust_dataset.csv")
    .then((res) => res.text())
    .then((csv) => {
...
function renderParetoBrand() {
  ...
  PROCESSED_ROWS.forEach(r => {
    ...
    if (!brandMap.has(brand)) {
      brandMap.set(brand, { name: brand, sumPrice: 0, sumSIS: 0, count: 0, paretoScore: 0 });
    }
...
function renderRecommendationTab(tabId, items) {
  ...
  items.forEach((item, idx) => {
    ...
    const paretoScore = item.paretoScore || 0;
    ...
    div.innerHTML = `
      <div class="reco-header">
        ...
        <div class="reco-score">Pareto: ${paretoScore.toFixed(3)}</div>
      </div>
      ...
      <div class="reco-note">Note: Pareto-optimal solutions are not ranked by single metric</div>
    `;
...
/* ========== EDA CHARTS (Section 5) - FIXED WITH DUAL-AXIS ========== */
...
function renderEdaMaterial() {
  ...
  const xScaleSIS = d3.scaleLinear().domain([0, d3.max(data, d => d.meanSIS) * 1.1]).range([padding.left, width - padding.right]);

  // Bars for Price (green)
  ...
  // Axes
  svg.append("g").attr("transform", `translate(0,${height - padding.bottom})`).call(d3.axisBottom(xScalePrice).ticks(5)).selectAll("text").style("font-size", "10px").attr("fill", "#10b981");
  svg.append("g").attr("transform", `translate(${padding.left},0)`).call(d3.axisLeft(yScale)).selectAll("text").style("font-size", "10px");
  
  // Right axis for SIS
  svg.append("g").attr("transform", `translate(${width - padding.right},0)`).call(d3.axisRight(xScaleSIS).ticks(5)).selectAll("text").style("font-size", "10px").attr("fill", "#059669");

  // Legend
  svg.append("text").attr("x", width / 2).attr("y", height - 5).attr("text-anchor", "middle").attr("font-size", "10px").attr("fill", "#10b981").text("Price (USD) — Bars");
  svg.append("text").attr("x", width - padding.right + 5).attr("y", padding.top - 5).attr("text-anchor", "start").attr("font-size", "10px").attr("fill", "#059669").text("SIS — Dots");
...
function renderEdaTrend() {
  ...
  // Axes
  svg.append("g").attr("transform", `translate(0,${height - padding.bottom})`).call(d3.axisBottom(xScaleCount).ticks(5)).selectAll("text").style("font-size", "10px").attr("fill", "#10b981");
  svg.append("g").attr("transform", `translate(${padding.left},0)`).call(d3.axisLeft(yScale)).selectAll("text").style("font-size", "10px");
  ...
  // Legend
  svg.append("text").attr("x", width / 2).attr("y", height - 5).attr("text-anchor", "middle").attr("font-size", "10px").attr("fill", "#10b981").text("Count — Bars");
  svg.append("text").attr("x", width - padding.right + 5).attr("y", padding.top - 5).attr("text-anchor", "start").attr("font-size", "10px").attr("fill", "#059669").text("SIS — Dots");
...
function renderEdaYear() {
  ...
  // Line for Price (green solid)
  const linePrice = d3.line().x(d => xScale(d.year)).y(d => yScalePrice(d.meanPrice));
  svg.append("path").datum(data).attr("fill", "none").attr("stroke", "#10b981").attr("stroke-width", 2).attr("d", linePrice);

  // Line for SIS (green dashed)
  const lineSIS = d3.line().x(d => xScale(d.year)).y(d => yScaleSIS(d.meanSIS));
  svg.append("path").datum(data).attr("fill", "none").attr("stroke", "#059669").attr("stroke-width", 2).attr("stroke-dasharray", "5,3").attr("d", lineSIS);
  ...
  svg.append("g").attr("transform", `translate(${padding.left},0)`).call(d3.axisLeft(yScalePrice).ticks(5)).selectAll("text").style("font-size", "10px").attr("fill", "#10b981");

  // Legend
  svg.append("line").attr("x1", width - 150).attr("y1", 15).attr("x2", width - 120).attr("y2", 15).attr("stroke", "#10b981").attr("stroke-width", 2);
  svg.append("text").attr("x", width - 115).attr("y", 19).attr("font-size", "10px").attr("fill", "#10b981").text("Price");
  
  svg.append("line").attr("x1", width - 150).attr("y1", 30).attr("x2", width - 120).attr("y2", 30).attr("stroke", "#059669").attr("stroke-width", 2).attr("stroke-dasharray", "5,3");
  svg.append("text").attr("x", width - 115).attr("y", 34).attr("font-size", "10px").attr("fill", "#059669").text("SIS");

  if (insightDiv && data.length > 0) {
    const latest = data[data.length - 1];
    insightDiv.innerHTML = `Year <strong>${latest.year}</strong>: avg price $${latest.meanPrice.toFixed(2)}, avg SIS ${latest.meanSIS.toFixed(3)}. Trend shows sustainability evolution over time.`;
  }
}

/* ========== MLP PREDICTION (Section 9) - FIXED ========== */
  ...
  predictBtn.addEventListener("click", () => {
    ...
    try {
      ...
      if (rating !== null && rating !== undefined) {
        predictionResult.innerHTML = `<strong>Predicted Sustainability Rating:</strong> ${rating.toFixed(4)} (based on provided metrics)`;
