 ...
 * 
 * Formula:
 * 1. Normalize: p_ij = x_ij / sum(x_i)
 * 2. Entropy: E_j = -k * sum(p_ij * ln(p_ij)), where k = 1/ln(n)
 * 3. Diversity: d_j = 1 - E_j
 * 4. Weight: w_j = d_j / sum(d_j)
 * 
 * @param {Array} envScores - Environmental scores (already normalized [0,1])
 * @param {Array} policyScores - Policy scores (already normalized [0,1])
 * @returns {Object} { weights: [w_env, w_policy], entropy: [E_env, E_policy], diversity: [d_env, d_policy] }
 */
...
function normalizeAndComputeSIS(rows) {
  ...
  // Step 1: Compute environmental_score and policy_score
  ...
  console.log(`Environmental Entropy: ${entropyWeights.entropy[0].toFixed(4)}`);
  console.log(`Policy Entropy: ${entropyWeights.entropy[1].toFixed(4)}`);
  console.log(`Environmental Diversity: ${entropyWeights.diversity[0].toFixed(4)}`);
  console.log(`Policy Diversity: ${entropyWeights.diversity[1].toFixed(4)}`);
...
function buildRecommendations(rowsWithSIS, priorityWeight, topN = 10) {
  ...
  const baseItems = rowsWithSIS.map((r, i) => {
    ...
    const finalScore =
      priorityWeight * sustainabilityTerm + (1 - priorityWeight) * priceTerm;
  ...
  return baseItems.sort((a, b) => b.finalScore - a.finalScore).slice(0, topN);
}

/* ========== PCA-LIKE PROJECTION ========== */

function computePcaLikeCoords(materialAgg) {
  const carbonVals = materialAgg.map((m) => m.meanCarbon);
  const waterVals = materialAgg.map((m) => m.meanWater);
  const wasteVals = materialAgg.map((m) => m.meanWaste);

  const carbonNorm = normalizeMinMax(carbonVals);
  const waterNorm = normalizeMinMax(waterVals);
  const wasteNorm = normalizeMinMax(wasteVals);
