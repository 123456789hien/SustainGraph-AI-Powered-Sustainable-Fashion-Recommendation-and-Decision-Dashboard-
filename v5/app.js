...
export const normalizeAndComputeSIS = (rows) => {
  ...
  ['Carbon_Footprint', 'Water_Usage', 'Waste_Generation', 'Overall_Sustainability_Score'].forEach((col) => {
  ...
  // Compute Sustainability Index Score (SIS)
  const rowsWithSIS = cleaned.map((r) => {
    const scoreNorm = r.Overall_Sustainability_Score / 5 || 0.5;
