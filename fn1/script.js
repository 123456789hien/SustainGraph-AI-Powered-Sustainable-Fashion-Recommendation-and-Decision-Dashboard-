...
loadSampleBtn.addEventListener("click", () => {
  ...
  fetch("https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/fn1/Kaggle_sust_dataset.csv")
    .then((res) => res.text())
    .then((csv) => {
...
function buildAndRenderRecommendations() {
  ...
  // Sort all categories by Pareto score descending
  const sortByPareto = (items) => items.sort((a, b) => (b.paretoScore || 0) - (a.paretoScore || 0));

  renderRecommendationTab('max-sustainability', sortByPareto(maxSustainability));
  renderRecommendationTab('best-value', sortByPareto(bestValue));
  renderRecommendationTab('balanced', sortByPareto(balanced));
...
function renderRecommendationTab(tabId, items) {
  ...
  items.forEach((item, idx) => {
    ...
    div.innerHTML = `
      ...
      <div class="reco-note">Note: Sorted by Pareto score (combines SIS and Price optimally)</div>
    `;
