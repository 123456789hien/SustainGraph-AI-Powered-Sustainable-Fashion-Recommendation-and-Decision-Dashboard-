...
const AUTOLOAD_URL =
  "https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/f1/Kaggle_sust_dataset.csv";
...
function renderPcaScatter() {
  ...
  svg
    ...
    .attr("fill", "#4b5563")
    .attr("font-size", 11)
    .text("Normalized CO₂, Water & Waste index per material");
...
/* 1) Material_Type vs SIS & Price */

function renderEdaMaterialChart() {
  const agg = MATERIAL_AGG;
  if (!agg.length) return;

  const prices = agg.map((m) => m.meanPrice || 0);
  const priceNorm = normalizeMinMax(prices).norm;

  const data = agg
    .map((m, i) => ({
      ...m,
      priceNorm: priceNorm[i],
    }))
  ...
  svg
    ...
    .attr("width", barWidth)
    .attr("height", (d) => yScale(0) - yScale(d.meanSIS))
    .attr("fill", "#22c55e")
    .append("title")
    .text((d) => `${d.Material_Type}\nSIS: ${d.meanSIS.toFixed(2)}`);

  // Price bars (normalized)
  svg
    .selectAll(".bar-price")
    .data(data)
    .enter()
    ...
    .attr("x", (d) => xScale(d.Material_Type) + barWidth + 2)
    .attr("y", (d) => yScale(d.priceNorm))
    .attr("width", barWidth)
    .attr("height", (d) => yScale(0) - yScale(d.priceNorm))
    .attr("fill", "#0ea5e9")
    .append("title")
    .text(
      (d) =>
        `${d.Material_Type}\nAvg Price: $${d.meanPrice.toFixed(
          2
        )}\nPrice (normalized): ${d.priceNorm.toFixed(2)}`
    );
  ...
  svg
    ...
    .attr("fill", "#4b5563")
    .attr("font-size", 11)
    .text("Green: SIS (0–1), Blue: Price (normalized 0–1)");
}

/* 2) Top countries by Price & SIS */
...
function renderEdaCountryChart() {
  ...
  svg
    ...
    .attr("height", (d) => yScale(0) - yScale(d.meanPrice))
    .attr("fill", (d) => colorScale(d.meanSIS))
    .append("title")
    .text(
      (d) =>
        `${d.Country}\nAvg Price: $${d.meanPrice.toFixed(
  ...
  svg
    ...
    .attr("fill", "#4b5563")
    .attr("font-size", 11)
    .text("Bar height: Avg Price, Color: Avg SIS (darker = more sustainable)");
}

/* 3) Trend — Count, SIS & Price */
...
function renderEdaTrendChart() {
  ...
  svg
    ...
    .attr("fill", "#4b5563")
    .attr("font-size", 11)
    .text("Bar height: count per trend (tooltip shows Avg SIS & Price)");
}
