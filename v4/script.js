import { parseCSVFile } from './logic/preprocess.js';
import { computeSIS } from './logic/computeSIS.js';
import { runKMeans } from './logic/kmeans.js';
import { runPCA2 } from './logic/pca.js';
import { recommend } from './logic/recommender.js';
...
runBtn.addEventListener('click', async () => {
  ...
  const { rowsWithSIS, stats: newStats } = computeSIS(filtered);
  processedRows = rowsWithSIS;
  stats = newStats;
  updateKPI();

  // Cluster materials using KMeans
  const materialMatrix = materialAgg.map(a => [a.meanCarbon, a.meanWater, a.meanWaste, a.meanScore]);
