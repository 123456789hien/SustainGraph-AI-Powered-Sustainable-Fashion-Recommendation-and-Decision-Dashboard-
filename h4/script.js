...
loadSampleBtn.addEventListener("click", () => {
  ...
  fetch("https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/h4/Kaggle_sust_dataset.csv")
    .then((res) => res.text())
    .then((csv) => {
...
// FIXED: computeYearAgg
function computeYearAgg(rows) {
  const map = new Map();
  rows.forEach(r => {
    const y = r.Year; // r.Year is now a number from app.js
...
// FIXED: computeTrendAgg was correct, but I'll put it here for completeness
...
// NEW: renderEdaTrend (Market Trend — Count & SIS)
function renderEdaTrend() {
  ...
  if (agg.length === 0) {
    container.innerHTML = "<p class='plot-nodata'>No data</p>";
    insightDiv.innerHTML = "<p>Không có dữ liệu xu hướng thị trường để phân tích.</p>";
    return;
  }

  // Insight calculation
  const mostFrequent = agg[0];
  const highestSIS = agg.reduce((max, item) => (item.meanSIS > max.meanSIS ? item : max), agg[0]);

  insightDiv.innerHTML = `
    <p><strong>Market Trend Analysis:</strong> Trends analyzed: ${agg.map(t => t.Trend).join(', ')}.</p>
    <p><strong>Insight:</strong> Trend <strong>${mostFrequent.Trend}</strong> is the most frequent (${mostFrequent.count} items). Trend <strong>${highestSIS.Trend}</strong> has the highest average SIS (${highestSIS.meanSIS.toFixed(3)}).</p>
    <p><strong>Recommendation:</strong> Align product development with high-SIS trends for market advantage.</p>
  `;

  // D3 Bar Chart (Count) and Line Chart (SIS)
  container.innerHTML = "";
  const width = container.clientWidth || 400;
  const height = container.clientHeight || 220;
  ...
  const xScale = d3.scaleBand().domain(agg.map(d => d.Trend)).range([padding.left, width - padding.right]).padding(0.3);
  ...
  svg.append("g").attr("transform", `translate(${padding.left},0)`).call(d3.axisLeft(yScaleCount).ticks(5).tickFormat(d3.format("d"))).selectAll("text").style("font-size", "10px");
  svg.append("g").attr("transform", `translate(${width - padding.right},0)`).call(d3.axisRight(yScaleSIS).ticks(5)).selectAll("text").style("font-size", "10px");

  // Labels
  svg.append("text").attr("x", width / 2).attr("y", height - 5).attr("text-anchor", "middle").attr("fill", "var(--text-muted)").attr("font-size", "11px").text("Market Trend");
  svg.append("text").attr("transform", "rotate(-90)").attr("x", -height / 2).attr("y", 15).attr("text-anchor", "middle").attr("fill", "var(--text-muted)").attr("font-size", "11px").text("Count (Left Axis)");
  svg.append("text").attr("transform", "rotate(90)").attr("x", height / 2).attr("y", -width + 45).attr("text-anchor", "middle").attr("fill", "var(--text-muted)").attr("font-size", "11px").text("Average SIS (Right Axis)");

  // Bars (Count)
  svg.selectAll(".bar-count").data(agg).enter().append("rect").attr("class", "bar-count")
  ...
  // Line (SIS)
  const lineSIS = d3.line().x(d => xScale(d.Trend) + xScale.bandwidth() / 2).y(d => yScaleSIS(d.meanSIS));
  svg.append("path").datum(agg).attr("fill", "none").attr("stroke", "var(--green-line)").attr("stroke-width", 2).attr("d", lineSIS);
  ...
  svg.selectAll(".dot-sis").data(agg).enter().append("circle").attr("class", "dot-sis")
    ...
    .on("mouseover", function (event, d) {
      d3.select(this).attr("r", 6);
      tooltip.html(`
        <div class="tooltip-header">Trend: ${d.Trend}</div>
        <div class="tooltip-body">
          <div><strong>Count:</strong> ${d.count}</div>
...
// NEW: renderEdaYear (Year — Price & SIS)
function renderEdaYear() {
  ...
  svg.append("text").attr("transform", "rotate(-90)").attr("x", -height / 2).attr("y", 15).attr("text-anchor", "middle").attr("fill", "var(--text-muted)").attr("font-size", "11px").text("Average Price (USD) (Left Axis)");
  svg.append("text").attr("transform", "rotate(90)").attr("x", height / 2).attr("y", -width + 45).attr("text-anchor", "middle").attr("fill", "var(--text-muted)").attr("font-size", "11px").text("Average SIS (Right Axis)");
  ...
  svg.selectAll(".dot-price").data(agg).enter().append("circle").attr("class", "dot-price")
...
// Initial call to load the sample data on page load
// This is a common pattern for client-side dashboards
document.addEventListener("DOMContentLoaded", () => {
  // Check if the auto-upload button exists and click it to load data immediately
