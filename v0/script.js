...
const edaCountryTop = document.getElementById("eda-country-top");
...
const AUTOLOAD_URL =
  "https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/v0/Kaggle_sust_dataset.csv";
...
/* ========== MATERIAL FILTER INTERACTIONS ========== */

// When material selection changes: auto-update EDA and Recommendations (for that material)
materialFilter.addEventListener("change", async () => {
  const mat = materialFilter.value;
  if (!mat || mat === "__all") {
    // reset to default country breakdown & recommendations (based on PROCESSED_ROWS if exist)
    renderCountryBar(); // default country counts or current processed
    if (PROCESSED_ROWS.length) {
      buildAndRenderRecommendations();
    } else {
      // no processed rows; clear recommendations
      recoList.innerHTML = "";
    }
    return;
  }

  // 1) Render country-by-material (mean SIS + avg price) and show top country
  renderCountryByMaterial(mat);

  // 2) Update recommendations to top brands for selected material
  // Use processed rows if available (more accurate), else compute quickly from RAW_ROWS
  const rowsSource = PROCESSED_ROWS.length ? PROCESSED_ROWS : normalizeAndComputeSIS(RAW_ROWS).rowsWithSIS;
  const rowsForMaterial = rowsSource.filter((r) => (r.Material_Type || "").trim() === mat);
  if (rowsForMaterial.length) {
    const w = parseFloat(prioritySelect.value || "0.5");
    const recs = buildRecommendations(rowsForMaterial, w, 10);
    renderRecommendations(recs);
  } else {
    recoList.innerHTML = "No recommendations for selected material.";
  }
});
...
function renderEDA() {
  ...
  // If material filter is set to a specific material, render material-country breakdown there
  if (materialFilter.value && materialFilter.value !== "__all") {
    renderCountryByMaterial(materialFilter.value);
  } else {
    renderCountryBar();
  }
}
...
// Bar chart: top countries count (default)
function renderCountryBar() {
  edaCountry.innerHTML = "";
  edaCountryTop.innerHTML = "";
  if (!PROCESSED_ROWS.length) {
...
/* ========== NEW: Country breakdown for a specific material (mean SIS, avg price) ========== */

function renderCountryByMaterial(material) {
  edaCountry.innerHTML = "";
  edaCountryTop.innerHTML = "";
  if (!RAW_ROWS.length && !PROCESSED_ROWS.length) {
    edaCountry.textContent = "No data available.";
    return;
  }

  // choose source: if PROCESSED_ROWS exist, use them (already have SIS); else compute quickly
  const sourceRows = PROCESSED_ROWS.length ? PROCESSED_ROWS : normalizeAndComputeSIS(RAW_ROWS).rowsWithSIS;

  // filter rows for the material
  const rows = sourceRows.filter((r) => (r.Material_Type || "").trim() === material);
  if (!rows.length) {
    edaCountry.textContent = `No rows for material "${material}".`;
    return;
  }

  // aggregate by country: mean SIS and mean price
  const map = new Map();
  rows.forEach((r) => {
    const c = r.Country || "Unknown";
    if (!map.has(c)) map.set(c, { sumSIS: 0, sumPrice: 0, count: 0 });
    const m = map.get(c);
    m.sumSIS += r.SIS || 0;
    m.sumPrice += isFinite(r.Average_Price_USD) ? r.Average_Price_USD : 0;
    m.count += 1;
  });
  ...
  // sort by meanSIS desc
  arr.sort((a, b) => b.meanSIS - a.meanSIS);
  const top = arr.slice(0, 10);

  // show top country summary text
  const topCountry = top[0];
  if (topCountry) {
    edaCountryTop.innerHTML = `<strong>Top country for material "${material}":</strong> ${topCountry.country} — mean SIS ${topCountry.meanSIS.toFixed(3)}, avg price $${(isFinite(topCountry.meanPrice) ? topCountry.meanPrice.toFixed(2) : "N/A")}.`;
  }

  // render bar chart of meanSIS
  const width = edaCountry.clientWidth || 700;
  const height = edaCountry.clientHeight || 220;
  const padding = 30;
  ...
  const yScale = d3
    .scaleLinear()
    .domain([0, d3.max(top, (d) => d.meanSIS) || 1])
    .nice()
    .range([height - padding, padding]);
  ...
  svg
    ...
    .append("rect")
    .attr("x", (d) => xScale(d.country))
    .attr("y", (d) => yScale(d.meanSIS))
    .attr("width", xScale.bandwidth())
    .attr("height", (d) => height - padding - yScale(d.meanSIS))
    .attr("fill", "#0f5035")
    .attr("opacity", 0.9)
    .append("title")
    .text((d) => `${d.country}: mean SIS ${d.meanSIS.toFixed(3)} — avg price $${(isFinite(d.meanPrice) ? d.meanPrice.toFixed(2) : "N/A")}`);

  // also add a small overlay line showing avg price (normalized) for context
  const prices = top.map((d) => d.meanPrice || 0);
  const pNorm = normalizeMinMax(prices);
  const lineYScale = d3.scaleLinear().domain([0, 1]).range([height - padding, padding]);

  const line = d3.line()
    .x((d) => xScale(d.country) + xScale.bandwidth() / 2)
    .y((d, i) => lineYScale(pNorm.norm[i]));
  ...
  // legend
  const legend = svg.append("g").attr("transform", `translate(${width - padding - 160}, ${padding})`);
  legend.append("rect").attr("width", 12).attr("height", 12).attr("fill", "#0f5035");
  legend.append("text").attr("x", 18).attr("y", 10).attr("font-size", 11).text("Mean SIS");
  legend.append("rect").attr("x", 0).attr("y", 18).attr("width", 12).attr("height", 12).attr("fill", "#f97316");
  legend.append("text").attr("x", 18).attr("y", 28).attr("font-size", 11).text("Avg Price (normalized)");
}
