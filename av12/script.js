...
loadSampleBtn.addEventListener("click", () => {
  ...
  fetch("https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/av12/Kaggle_sust_dataset.csv")
    .then((res) => res.text())
    .then((csv) => {
...
function renderClusterScatterInsight(kUsed) {
  ...
  // Find the cluster with the highest mean SIS
  const clusterSIS = MATERIAL_AGG.reduce((acc, m) => {
      acc[m.cluster] = (acc[m.cluster] || { sumSIS: 0, count: 0 });
      acc[m.cluster].sumSIS += m.meanSIS;
      acc[m.cluster].count += 1;
      return acc;
  }, {});
  
  const meanSISByCluster = Object.entries(clusterSIS).map(([cluster, data]) => ({
      cluster: cluster,
      meanSIS: data.sumSIS / data.count
  }));
  
  const bestCluster = meanSISByCluster.sort((a, b) => b.meanSIS - a.meanSIS)[0];
  
  const largestClusterEntry = Object.entries(clusterCounts).sort(
    ([, a], [, b]) => b - a
  )[0];
  
  const largestCluster = largestClusterEntry ? largestClusterEntry[0] : 'N/A';
  const largestClusterCount = largestClusterEntry ? largestClusterEntry[1] : 0;
  ...
  insightDiv.innerHTML = `
    ...
    <p><strong>Insight:</strong> Cluster ${largestCluster} is the largest with ${largestClusterCount} materials. Cluster <strong>${bestCluster.cluster}</strong> has the highest average SIS (${bestCluster.meanSIS.toFixed(3)}), indicating the most sustainable materials.</p>
    <p><strong>Recommendation:</strong> Focus on materials in the cluster with the highest average SIS for the most sustainable sourcing options.</p>
  `;
}
...
function renderClusterLegend(kUsed) {
  ...
  for (let i = 0; i < kUsed; i++) {
    ...
    div.className = "cluster-pill"; // Changed from legend-item to cluster-pill for consistent styling
    div.innerHTML = `<span class="cluster-dot" style="background-color:${colorScale(i)};"></span> Cluster ${i}`;
...
function renderEdaTrend() {
  ...
  const mostSustainable = agg.reduce((max, item) => (item.meanSIS > max.meanSIS ? item : max), agg[0]);
  const largestTrend = agg[0];

  insightDiv.innerHTML = `
    <p><strong>Market Trends:</strong> ${agg.map(t => t.Trend).join(', ')}.</p>
    <p><strong>Insight:</strong> Trend <strong>${largestTrend.Trend}</strong> is the most frequent (${largestTrend.count} items). Trend <strong>${mostSustainable.Trend}</strong> has the highest average SIS (${mostSustainable.meanSIS.toFixed(3)}).</p>
    <p><strong>Recommendation:</strong> Align product development with high-SIS trends for market advantage.</p>
  `;

  // D3 Bar Chart (Count vs SIS)
  container.innerHTML = "";
  const width = container.clientWidth || 400;
  const height = container.clientHeight || 220;
  const padding = { top: 10, right: 10, bottom: 50, left: 50 };
  ...
  const xScale = d3.scaleBand().domain(agg.map(d => d.Trend)).range([padding.left, width - padding.right]).padding(0.1);
  ...
  // Labels
  svg.append("text").attr("transform", "rotate(-90)").attr("x", -height / 2).attr("y", 15).attr("text-anchor", "middle").attr("fill", "var(--text-muted)").attr("font-size", "11px").text("Count");
  svg.append("text").attr("transform", "rotate(90)").attr("x", height / 2).attr("y", -width + 5).attr("text-anchor", "middle").attr("fill", "var(--text-muted)").attr("font-size", "11px").text("Average SIS");
  ...
  // Line (SIS)
  const lineSIS = d3.line().x(d => xScale(d.Trend) + xScale.bandwidth() * 0.75).y(d => yScaleSIS(d.meanSIS));
  svg.append("path").datum(agg).attr("fill", "none").attr("stroke", "var(--green-line)").attr("stroke-width", 2).attr("d", lineSIS);

  // Points (SIS)
  const tooltip = createTooltip("eda-trend-tooltip");
  svg.selectAll(".dot-sis").data(agg).enter().append("circle").attr("class", "dot-sis")
    .attr("cx", d => xScale(d.Trend) + xScale.bandwidth() * 0.75)
    .attr("cy", d => yScaleSIS(d.meanSIS))
    .attr("r", 4)
    .attr("fill", "var(--green-line)")
    .attr("stroke", "#fff")
