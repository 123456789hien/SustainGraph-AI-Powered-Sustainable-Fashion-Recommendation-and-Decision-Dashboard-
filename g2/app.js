/* app.js
   Core logic for SustainGraph: parsing CSV, computing SIS, clustering, EDA aggregates,
   ML regression features and helpers for LSTM input.
...
function parseCSVText(text) {
  // Chuẩn hóa xuống dòng rồi split
  const lines = text
    .replace(/\r\n/g, "\n")
    .split(/\n/)
    .filter((l) => l.trim() !== "");
...
function normalizeAndComputeSIS(rawRows) {
  ...
  const rowsWithSIS = rows.map((r, i) => {
    ...
    const SIS = (envScore * 0.7 + policyScore * 0.3).toFixed(4) * 1;
...
/* ========== SIMPLE KMEANS (3 CLUSTERS, SAFE FOR N < K) ========== */
...
/* ========== "PCA" PROJECTION (Carbon vs avg(Water, Waste)) ========== */
...
/* ========== EDA AGGREGATES (countries, trends, years) ========== */
...
/* ========== ML Regression Features (for SIS) ========== */

function buildSISRegressionXY(rowsWithSIS) {
  const xs = [];
  const ys = [];
  rowsWithSIS.forEach((r) => {
    if (!isFinite(r.SIS)) return;
    const feat = [
      r.Carbon_Footprint_norm ?? 0.5,
      r.Water_Usage_norm ?? 0.5,
      r.Waste_Production_norm ?? 0.5,
      r.ratingScore ?? 0.5,
      r.ecoScore ?? 0.5,
      r.recScore ?? 0.5,
      r.Price_norm ?? 0.5,
    ];
    if (feat.some((v) => !isFinite(v))) return;
    xs.push(feat);
    ys.push(r.SIS);
  });
  return { xs, ys };
...
/* LSTM helpers — từ YearAgg -> windows */

function buildLSTMWindowsFromYearAgg(yearAgg, windowSize) {
  const values = yearAgg.map((y) => y.meanSIS);
  if (values.length <= windowSize) return { xs: [], ys: [], years: [] };
  ...
  const years = yearAgg.map((y) => y.Year);
  return { xs, ys, years };
}
