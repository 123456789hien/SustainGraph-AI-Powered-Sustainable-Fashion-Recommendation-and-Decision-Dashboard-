...
loadSampleBtn.addEventListener("click", () => {
  ...
  fetch("https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/l1/Kaggle_sust_dataset.csv")
    .then((res) => res.text())
    .then((csv) => {
...
autoUploadBtn.addEventListener("click", () => {
  loadStatus.textContent = "Loading sample data...";
  fetch("https://raw.githubusercontent.com/123456789hien/SustainGraph-AI-Powered-Sustainable-Fashion-Recommendation-and-Decision-Dashboard-/refs/heads/main/ll1/Kaggle_sust_dataset.csv")
    .then((res) => res.text())
    .then((csv) => {
      const file = new File([csv], "sample.csv", { type: "text/csv" });
      parseCsv(file, true);
...
function renderDataQualityValidation() {
  ...
  dataQualityDiv.innerHTML = `
    <h3 style="margin-top: 0;">Scientific Justification for Entropy Weighting</h3>
    <p>
      The Entropy Weight Method (EWM) is used to objectively determine the weights of the Environmental and Policy scores based on their <strong>information content</strong> (variance) within the dataset.
    </p>
    ...
    <div class="validation-insight">
      <strong>Academic Defense (Yellow Insight Box):</strong><br>
      The dataset exhibits <strong>low variance</strong> in environmental footprints (Carbon, Water, Waste), resulting in a <strong>high entropy</strong> for the Environmental Score. Conversely, the Policy Score (based on Rating and Recycling) shows <strong>higher variance</strong>, leading to a <strong>lower entropy</strong> and thus a <strong>higher objective weight (${ENTROPY_WEIGHTS.policy.toFixed(4)})</strong> compared to the Environmental Score (${ENTROPY_WEIGHTS.env.toFixed(4)}).
      <br><br>
      <strong>Warning on Generalization:</strong> The resulting Sustainability Index Score (SIS) is highly dependent on the Policy Score due to this data characteristic. Caution must be exercised when generalizing these findings to datasets with higher environmental variance.
    </div>
  `;
}
...
function renderClusterInterpretationTable(kUsed) {
  ...
  tableHTML += `
    ...
    <p style="margin-top: 10px; font-size: 12px; color: var(--text-muted); font-style: italic;">
      <strong>Interpretation Note:</strong> Cluster separation is primarily driven by policy scores (Avg Policy Score, Avg Rating, Avg Recycling) due to low environmental variance in the dataset.
    </p>
    <ul>
      <li><strong>High-Impact Cluster:</strong> Characterized by high policy compliance (e.g., high Avg Rating, high Avg Recycling) and high Avg SIS.</li>
      <li><strong>Low-Impact Cluster:</strong> Characterized by low policy compliance and low Avg SIS.</li>
      <li><strong>Environmental Metrics:</strong> Environmental scores (Avg Env Score) are relatively uniform across clusters, reinforcing the Data Quality Validation finding.</li>
    </ul>
  `;
...
function renderClusterScatter(kUsed) {
  ...
  svg.selectAll("circle").data(data).enter().append("circle")
    ...
    .on("mouseover", function (event, d) {
      ...
      const clusterName = clusterNames[d.cluster] || `Cluster ${d.cluster}`;
      tooltip.html(`
...
function renderRecommendationTab(tabId, items) {
  ...
  // Note: "Pareto-optimal solutions are not ranked" - remove rank badge
  items.forEach((item) => {
    ...
    div.innerHTML = `
      ...
      <div class="reco-pill-row">
        ...
        ${tabId === 'max-sustainability' ? '<span class="reco-pill">Max Sustainability</span>' : ''}
        ${tabId === 'best-value' ? '<span class="reco-pill">Best Value</span>' : ''}
        ${tabId === 'balanced' ? '<span class="reco-pill">Balanced Trade-off</span>' : ''}
        <span class="reco-pill">${material}</span>
      </div>
    `;
...
function renderEdaMaterial() {
  ...
  container.innerHTML = "";
  const data = MATERIAL_AGG.sort((a, b) => b.meanPrice - a.meanPrice).slice(0, 10);
  ...
  const xScale = d3.scaleBand().domain(data.map(d => d.Material_Type)).range([padding.left, width - padding.right]).padding(0.2);
  const yScalePrice = d3.scaleLinear().domain([0, d3.max(data, d => d.meanPrice) * 1.1]).range([height - padding.bottom, padding.top]);
  const yScaleSIS = d3.scaleLinear().domain([0, d3.max(data, d => d.meanSIS) * 1.1]).range([height - padding.bottom, padding.top]);
  ...
  svg.append("text").attr("transform", "rotate(-90)").attr("y", 0).attr("x", 0 - (height / 2)).attr("dy", "1em").style("text-anchor", "middle").text("Price").attr("font-size", "12px").attr("fill", "#666");
  svg.append("text").attr("transform", "rotate(-90)").attr("y", width - 5).attr("x", 0 - (height / 2)).attr("dy", "1em").style("text-anchor", "middle").text("SIS").attr("font-size", "12px").attr("fill", "#666");

  const tooltip = createTooltip("eda-material-tooltip");

  svg.selectAll(".bar").data(data).enter().append("rect").attr("class", "bar")
    .attr("x", d => xScale(d.Material_Type)).attr("y", d => yScalePrice(d.meanPrice)).attr("width", xScale.bandwidth()).attr("height", d => height - padding.bottom - yScalePrice(d.meanPrice))
    .attr("fill", "#2E8B57")
    .on("mouseover", function (event, d) {
      d3.select(this).attr("fill", "#1E6A42");
      tooltip.html(`<strong>${d.Material_Type}</strong><br>Price: $${d.meanPrice.toFixed(2)}<br>SIS: ${d.meanSIS.toFixed(3)}`).style("visibility", "visible");
    })
    .on("mousemove", (event) => tooltip.style("top", (event.pageY - 10) + "px").style("left", (event.pageX + 10) + "px"))
  ...
  svg.selectAll(".dot").data(data).enter().append("circle").attr("class", "dot")
    .attr("cx", d => xScale(d.Material_Type) + xScale.bandwidth() / 2).attr("cy", d => yScaleSIS(d.meanSIS)).attr("r", 5)
    .attr("fill", "#3CB371")
    .on("mouseover", function (event, d) {
      d3.select(this).attr("r", 7);
      tooltip.html(`<strong>${d.Material_Type}</strong><br>Price: $${d.meanPrice.toFixed(2)}<br>SIS: ${d.meanSIS.toFixed(3)}`).style("visibility", "visible");
  ...
  const legend = svg.append("g").attr("font-family", "sans-serif").attr("font-size", 10).attr("text-anchor", "end").selectAll("g").data(["Price", "SIS"]).enter().append("g").attr("transform", (d, i) => `translate(0,${i * 20})`);
  legend.append("rect").attr("x", width - 19).attr("width", 19).attr("height", 19).attr("fill", (d, i) => i === 0 ? "#2E8B57" : "#3CB371");
  legend.append("text").attr("x", width - 24).attr("y", 9.5).attr("dy", "0.32em").text(d => d);

  if (insightDiv && data.length > 0) {
    const best = data.sort((a, b) => b.meanSIS - a.meanSIS)[0];
...
function renderEdaCountry() {
  ...
  const data = Array.from(countryMap.values()).map(c => ({
    ...
    count: c.count
  })).sort((a, b) => b.meanPrice - a.meanPrice).slice(0, 10);
  ...
  const xScale = d3.scaleBand().domain(data.map(d => d.country)).range([padding.left, width - padding.right]).padding(0.2);
  const yScalePrice = d3.scaleLinear().domain([0, d3.max(data, d => d.meanPrice) * 1.1]).range([height - padding.bottom, padding.top]);
  const yScaleSIS = d3.scaleLinear().domain([0, d3.max(data, d => d.meanSIS) * 1.1]).range([height - padding.bottom, padding.top]);
  ...
  svg.selectAll(".bar").data(data).enter().append("rect").attr("class", "bar")
    ...
    .attr("fill", "#2E8B57") // Green bars (Price)
    .on("mouseover", function (event, d) {
      d3.select(this).attr("fill", "#1E6A42");
      tooltip.html(`<strong>${d.country}</strong><br>Price: $${d.meanPrice.toFixed(2)}<br>SIS: ${d.meanSIS.toFixed(3)}`).style("visibility", "visible");
    })
    .on("mousemove", (event) => tooltip.style("top", (event.pageY - 10) + "px").style("left", (event.pageX + 10) + "px"))
    .on("mouseout", function (d) {
      d3.select(this).attr("fill", "#2E8B57");
  ...
  svg.selectAll(".dot").data(data).enter().append("circle").attr("class", "dot")
    ...
    .attr("r", 5)
    .attr("fill", "#3CB371") // Green dots (SIS)
    .on("mouseover", function (event, d) {
      d3.select(this).attr("r", 7);
      tooltip.html(`<strong>${d.country}</strong><br>Price: $${d.meanPrice.toFixed(2)}<br>SIS: ${d.meanSIS.toFixed(3)}`).style("visibility", "visible");
    })
  ...
  if (insightDiv && data.length > 0) {
    const best = data.sort((a, b) => b.meanSIS - a.meanSIS)[0];
    insightDiv.innerHTML = `<strong>${best.country}</strong> leads with SIS ${best.meanSIS.toFixed(3)}, avg price $${best.meanPrice.toFixed(2)}. Top sustainable sourcing country.`;
  }
}
...
function renderEdaTrend() {
  ...
  const xScale = d3.scaleBand().domain(data.map(d => d.trend)).range([padding.left, width - padding.right]).padding(0.2);
  const yScaleCount = d3.scaleLinear().domain([0, d3.max(data, d => d.count) * 1.1]).range([height - padding.bottom, padding.top]);
  const yScaleSIS = d3.scaleLinear().domain([0, d3.max(data, d => d.meanSIS) * 1.1]).range([height - padding.bottom, padding.top]);
  ...
  svg.append("text").attr("transform", "rotate(-90)").attr("y", 0).attr("x", 0 - (height / 2)).attr("dy", "1em").style("text-anchor", "middle").text("Count").attr("font-size", "12px").attr("fill", "#666");
  svg.append("text").attr("transform", "rotate(-90)").attr("y", width - 5).attr("x", 0 - (height / 2)).attr("dy", "1em").style("text-anchor", "middle").text("SIS").attr("font-size", "12px").attr("fill", "#666");
  ...
  svg.selectAll(".bar").data(data).enter().append("rect").attr("class", "bar")
    ...
    .attr("fill", "#2E8B57") // Green bars (Count)
    .on("mouseover", function (event, d) {
      d3.select(this).attr("fill", "#1E6A42");
      tooltip.html(`<strong>${d.trend}</strong><br>Count: ${d.count}<br>SIS: ${d.meanSIS.toFixed(3)}`).style("visibility", "visible");
    })
    .on("mousemove", (event) => tooltip.style("top", (event.pageY - 10) + "px").style("left", (event.pageX + 10) + "px"))
    .on("mouseout", function (d) {
      d3.select(this).attr("fill", "#2E8B57");
  ...
  // Legend
  const legend = svg.append("g").attr("font-family", "sans-serif").attr("font-size", 10).attr("text-anchor", "end").selectAll("g").data(["Count", "SIS"]).enter().append("g").attr("transform", (d, i) => `translate(0,${i * 20})`);
  legend.append("rect").attr("x", width - 19).attr("width", 19).attr("height", 19).attr("fill", (d, i) => i === 0 ? "#2E8B57" : "#3CB371");
  legend.append("text").attr("x", width - 24).attr("y", 9.5).attr("dy", "0.32em").text(d => d);

  if (insightDiv && data.length > 0) {
    const mostCommon = data[0];
    const highestSIS = data.sort((a, b) => b.meanSIS - a.meanSIS)[0];
    insightDiv.innerHTML = `Most common trend is <strong>${mostCommon.trend}</strong> (${mostCommon.count} brands). Highest SIS is <strong>${highestSIS.trend}</strong> (${highestSIS.meanSIS.toFixed(3)}).`;
  }
}
...
function renderEdaYear() {
  ...
  // Price Line (Solid Green)
  svg.append("path").datum(data).attr("fill", "none").attr("stroke", "#2E8B57").attr("stroke-width", 2).attr("d", linePrice);

  // SIS Line (Dashed Green)
  svg.append("path").datum(data).attr("fill", "none").attr("stroke", "#3CB371").attr("stroke-width", 2).attr("stroke-dasharray", "5,5").attr("d", lineSIS);

  const tooltip = createTooltip("eda-year-tooltip");

  // Price Dots
  svg.selectAll("circle.price-dot").data(data).enter().append("circle").attr("class", "price-dot")
    .attr("cx", d => xScale(d.year)).attr("cy", d => yScalePrice(d.meanPrice)).attr("r", 4).attr("fill", "#2E8B57").attr("cursor", "pointer")
    .on("mouseover", function (event, d) {
      d3.select(this).attr("r", 6);
      tooltip.html(`<strong>Year ${d.year}</strong><br>Price: $${d.meanPrice.toFixed(2)}<br>SIS: ${d.meanSIS.toFixed(3)}`).style("visibility", "visible");
    })
    .on("mousemove", (event) => tooltip.style("top", (event.pageY - 10) + "px").style("left", (event.pageX + 10) + "px"))
    .on("mouseout", function () {
  ...
  // SIS Dots
  svg.selectAll("circle.sis-dot").data(data).enter().append("circle").attr("class", "sis-dot")
    .attr("cx", d => xScale(d.year)).attr("cy", d => yScaleSIS(d.meanSIS)).attr("r", 4).attr("fill", "#3CB371").attr("cursor", "pointer")
    .on("mouseover", function (event, d) {
      d3.select(this).attr("r", 6);
      tooltip.html(`<strong>Year ${d.year}</strong><br>Price: $${d.meanPrice.toFixed(2)}<br>SIS: ${d.meanSIS.toFixed(3)}`).style("visibility", "visible");
    })
  ...
  // Legend
  const legend = svg.append("g").attr("font-family", "sans-serif").attr("font-size", 10).attr("text-anchor", "end").selectAll("g").data(["Price (Solid Line)", "SIS (Dashed Line)"]).enter().append("g").attr("transform", (d, i) => `translate(0,${i * 20})`);
  legend.append("line").attr("x1", width - 40).attr("x2", width - 20).attr("y1", 9.5).attr("y2", 9.5).attr("stroke-width", 2).attr("stroke", (d, i) => i === 0 ? "#2E8B57" : "#3CB371").attr("stroke-dasharray", (d, i) => i === 1 ? "5,5" : "none");
  legend.append("text").attr("x", width - 45).attr("y", 9.5).attr("dy", "0.32em").text(d => d);

  if (insightDiv && data.length > 1) {
    const startSIS = data[0].meanSIS;
    const endSIS = data[data.length - 1].meanSIS;
    const trend = endSIS > startSIS ? "increased" : "decreased";
    insightDiv.innerHTML = `Avg SIS has <strong>${trend}</strong> from ${startSIS.toFixed(3)} in ${data[0].year} to ${endSIS.toFixed(3)} in ${data[data.length - 1].year}.`;
  }
}
...
const predictBtn = document.getElementById("predict-rating-btn");
...
if (predictBtn) {
  predictBtn.addEventListener("click", () => {
    ...
    // The recycling input is a number input in the form, so we need to get its value
    const recyclingInput = document.getElementById("pred-recycling");
    const recycling = recyclingInput ? parseFloat(recyclingInput.value) : 0;

    if (isNaN(carbon) || isNaN(water) || isNaN(waste) || isNaN(recycling)) {
      predictionResult.innerHTML = `<strong>Warning:</strong> Please fill in all fields with valid numbers!`;
