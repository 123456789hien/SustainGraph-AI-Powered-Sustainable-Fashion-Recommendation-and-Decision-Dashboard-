...
function buildRecommendations(rowsWithSIS, topN = 5) {
  ...
  paretoItems.forEach((item, i) => {
    // Remove old paretoScore calculation as it's replaced by the distanceToIdeal metric
    // This old calculation was flawed as per the teacher's feedback
    item.paretoScore = item.SIS; // Placeholder, will be replaced by distanceToIdeal in the new logic
    item.balanceScore = item.SIS; // Placeholder, will be replaced by distanceToIdeal in the new logic
  });

  // --- NEW ACADEMICALLY CORRECT PARETO LOGIC ---
  
  // Determine the number of items to select for each category (e.g., Top 20% or a fixed number)
  // Using a fixed number for now, but will use a percentage of the Pareto set size for better logic
  const paretoSetSize = paretoItems.length;
  const selectionSize = Math.max(5, Math.ceil(paretoSetSize * 0.2)); // Select at least 5 or 20%

  // 1. Max Sustainability: Top SIS in Pareto set
  // Sort by SIS descending and take the top selectionSize
  const maxSustainability = [...paretoItems]
    .sort((a, b) => b.SIS - a.SIS)
    .slice(0, selectionSize);

  // 2. Best Value: Lowest price in Pareto set
  // Sort by Price ascending and take the top selectionSize
  const bestValue = [...paretoItems]
    .sort((a, b) => a.Average_Price_USD - b.Average_Price_USD)
    .slice(0, selectionSize);

  // 3. Balanced Trade-off: Items with the smallest Distance-to-Ideal Metric (d)
  // First, calculate the Distance-to-Ideal Metric (d) for all Pareto items
  // d = sqrt( (1 - SIS_norm)^2 + (Price_norm)^2 )
  
  // Re-normalize SIS and Price locally for the Pareto set to ensure 0-1 range
  const sisNormGlobal = normalizeMinMax(paretoItems.map(item => item.SIS));
  const priceNormGlobal = normalizeMinMax(paretoItems.map(item => item.Average_Price_USD));

  paretoItems.forEach((item, i) => {
    const sisNorm = sisNormGlobal.norm[i];
    const priceNorm = priceNormGlobal.norm[i];
    
    // Calculate Distance-to-Ideal (d)
    // Ideal point is (SIS=1, Price=0) in normalized space
    item.distanceToIdeal = Math.sqrt(
      Math.pow(1 - sisNorm, 2) + Math.pow(priceNorm, 2)
    );
  });

  // Sort by Distance-to-Ideal ascending (smallest distance is best balance)
  const balanced = [...paretoItems]
    .sort((a, b) => a.distanceToIdeal - b.distanceToIdeal)
    .slice(0, selectionSize);
    
  // Ensure the final lists are unique (since the top selections might overlap)
  // This is a common issue in Pareto set analysis, but for display, we'll keep the lists as is
  // The user requested the original tab names, so we'll return the three distinct lists
  
  // Re-calculate the paretoScore for display, using the new balanceScore (distanceToIdeal)
  // We'll use the inverse of distanceToIdeal as the balanceScore for display sorting (higher is better)
  paretoItems.forEach(item => {
    item.balanceScore = 1 - item.distanceToIdeal; // Higher balanceScore is better
  });
  
  // Final sort for display within each category (optional, but good practice)
  maxSustainability.sort((a, b) => b.SIS - a.SIS);
  bestValue.sort((a, b) => a.Average_Price_USD - b.Average_Price_USD);
  balanced.sort((a, b) => b.balanceScore - a.balanceScore); // Sort by new balanceScore
